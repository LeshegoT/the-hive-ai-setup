...
xdescribe('routing tests', function() {
  ...
  it('the page selector switches pages', async function() {
    ...
    await testNavigation(page, 'tracks', 'tracks');
    await testNavigation(page, 'home', 'View One');
  });
  ...
  it('the page selector switches pages in a different way', async function() {
    ...
    await testNavigationInADifferentWay(page, 'tracks', 'tracks');
    await testNavigationInADifferentWay(page, 'home', 'View One');
  });
});
...
async function testNavigation(page, href, linkText) {
  ...
  const theHive = await page.$('the-hive');
  const myText = await page.evaluate(getShadowRootChildProp, theHive, selector, 'textContent');
  expect(await myText).equal(linkText);

  // Does the click take you to the right page?
  await page.evaluate(doShadowRootClick, theHive, selector);
...
async function testNavigationInADifferentWay(page, href, linkText) {
  const query = `the-hive::shadow a[href="/${href}"]`;
