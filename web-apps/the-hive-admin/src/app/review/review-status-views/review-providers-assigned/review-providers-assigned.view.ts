...
@Component({
    selector: 'app-review-providers-assigned',
    templateUrl: './review-providers-assigned.view.html',
    styleUrls: ['../../styles/reviewShared.css'],
    standalone: false
})
export class ReviewProvidersAssignedComponent extends FeedbackProvidersAssignable implements OnInit, OnChanges {
  ...
  setReviewData(filterParameters: UpcomingReviewsFilterParameters) {
    ...
    this.reviewStatusService.getReviewStatuses().subscribe((statuses) => {
      ...
      this.feedbackService.getReviews(this.assignProvidersStatusId, filterParameters).subscribe((reviews) => {
